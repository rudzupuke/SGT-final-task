        ...
        buddies.map((buddy) => {
          return (
            <div className="db-buddy-card" key={buddy.user_id}>
              <img className="db-buddy-card__img" src={buddy.picture} />
              <h2 className="db-buddy-card__heading">
                <span className="bold">{buddy.name}</span>, {buddy.age}
              </h2>
              <div className="db-buddy-card__btncont">
                <button className="button--buddies" onClick={goOnAWalk}>
                  Go on a walk!
                </button>
                <button className="button--buddies">Chat</button>
              </div>
            </div>
          );
        })}
      {showGoOnAWalkModal && (
        <GoOnAWalkModal setShowGoOnAWalkModal={setShowGoOnAWalkModal} />
      )}
    </>
  );
};
